...
fn loadBorders(
    ...
    var lines: std.ArrayList(BorderLine) = try .initCapacity(alloc, total_polylines);
    errdefer {
        for (lines.items) |line| alloc.free(line.points);
        lines.deinit(alloc);
    }

    for (0..total_polylines) |_| {
        ...
        lines.appendAssumeCapacity(.{ .points = points });
    }
